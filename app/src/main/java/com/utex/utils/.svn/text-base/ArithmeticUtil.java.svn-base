...
public class ArithmeticUtil {
     ...
     * @return
     */
    public static String Vol(String vol) {
        ...
        String substring;
        if (vol.indexOf(".") != -1) {
            substring = vol.substring(0, vol.indexOf("."));
        } else {
            substring = vol;
        }
        ...
        if (Long.parseLong(substring) > 1) {
            ...
            if (Long.parseLong(substring) <= 100000) {
                //保留 4位小数
                bigDecimal = round(vol, 4);
                return bigDecimal.toString();
            } else {
                //不显示小数，取整
                bigDecimal = round(vol, 0);
                if (Long.parseLong(substring) < 100000000) {
                    //取整，全部显示
                    return bigDecimal.toString();
                } else {
                    //显示 亿
                    BigDecimal div = div(vol, "100000000", 2);
                    return div.toString() + "亿";
                }
            }
        } else if (Long.parseLong(substring) < 1 && Long.parseLong(substring) > 0) {
            //小于0  保留八位
            bigDecimal = round(vol, 8);
            return bigDecimal.toString();
        } else {
            return "0";
        }
     ...
     * 科学技术发处理
     */
    public static String transformation(Double str) {

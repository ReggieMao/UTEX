...
import com.utex.mvp.marketdetail.model.IMarketDetailModel;
import com.utex.mvp.marketdetail.model.MarketDetailModel;
 ...
 */
public class MarketDetailPresenter implements IMarketDetailPresenter {

    private ApiService apiService;

    private IMarketDetailView iMarketDetailView;

    private IMarketDetailModel iMarketDetailModel;
    private List<KLineEntity> allKlineEntity;

    public MarketDetailPresenter(ApiService apiService, MarketDetailActivity activity) {
        this.apiService = apiService;
        this.iMarketDetailView = activity;
        iMarketDetailModel = new MarketDetailModel();
    }
    ...
    public void getCoinInfo(String coin) {
        iMarketDetailModel.getCoinInfo(apiService, coin, this);
    }

    @Override
    public void getCoinInfoSuccess(CoinBaseVO body) {
        iMarketDetailView.getCoinInfoSuccess(body);
    }

    @Override
    public void cannelOptional(String coinMarketCode) {
        iMarketDetailModel.cannelOptional(apiService, coinMarketCode, this);
    }

    @Override
    public void addOptional(String coinMarketCode, Long id) {
        iMarketDetailModel.addOptional(apiService, coinMarketCode, id, this);
    }

    @Override
    public void addOptionalSuccess() {
        iMarketDetailView.addOptionalSuccess();
    }

    @Override
    public void cannelOptionalSuccess() {
        iMarketDetailView.cannelOptionalSuccess();
    }
    ...
    @Override
    public void volSubscribe(TickerDo currTicker) {
        MarketSocketPackUtils.volSubscribe(currTicker, 3);
    }

    @Override
    public void depthSubscribe(String value, TickerDo currTicker) {
        MarketSocketPackUtils.depthSubscribe(value, currTicker, 3);
    }

    @Override
    public void coinSubscribe(TickerDo currTicker) {
        List<TickerDo> tickerDos = new ArrayList<>();
        tickerDos.add(currTicker);
        MarketSocketPackUtils.todaySubscribe(tickerDos, 3);
    }
    ...
    @Override
    public void volUnSubscribe() {
        MarketSocketPackUtils.volUnSubscribe(3);
    }

    @Override
    public void depthUnSubscribe() {
        MarketSocketPackUtils.depthUnSubscribe(3);
    }

    @Override
    public void todayUnSubscribe() {
        MarketSocketPackUtils.todayUnSubscribe(3);
    }

    @Override
    public void kLineUnSubscribe() {
        MarketSocketPackUtils.kLineUnSubscribe(3);
    }

package com.utex.mvp.order.dagger2;

import com.utex.base.BaseComponent;
import com.utex.core.ActivityScope;
import com.utex.core.AppComponent;
import com.utex.mvp.order.presenter.IOrderManagePresenter;
import com.utex.mvp.order.view.OrderManageFragment;

import dagger.Component;

/**
 * Created by admin on 2017/4/21.
 */
@ActivityScope
@Component(modules = OrderManageModule.class, dependencies = AppComponent.class)
public interface OrderManageComponent extends BaseComponent<OrderManageFragment, IOrderManagePresenter> {

}
